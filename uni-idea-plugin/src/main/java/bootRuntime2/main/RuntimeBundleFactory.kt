...
class RuntimeLocationsFactory {
  ...
  fun bintrayBundles(project: Project): List<Runtime> {
    ...
    val runtimes: List<Runtime> =
      collectRuntimes(String.format(linkTemplate, subject, repoName), project, ".*\"(jbsdk.*%s.*%s.*?)\"") +
        collectRuntimes(String.format(linkTemplate, subject, jbrRepoName), project, ".*\".*?(jbrsdk.*%s.*%s.*?)\"")

    //jbsdk8u202b1491_osx_x64.tar.gz
    Uni.log.debug { listOf("bintray jvm runtimes", runtimes) }
    return runtimes
  }

  private fun collectRuntimes(link: String,
                              project: Project,
                              bundleNamePattern: String): List<Runtime> {
    val response =  try {
      URL(link).readText()
