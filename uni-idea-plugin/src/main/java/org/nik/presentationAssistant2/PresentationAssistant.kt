  ...
  fun setShowActionsDescriptions(value: Boolean, project: Project?) {
    ...
    if (value && presenter == null) {
      presenter = ShortcutPresenter().apply {
        showActionInfo(
          ActionData(
            "org.nik.presentationAssistant.ShowActionDescriptions",
            project,
...
class PresentationAssistantConfigurable : Configurable, SearchableConfigurable {
  ...
  private val horizontalAlignmentButtons =
    PopupHorizontalAlignment.values().associate { it to JRadioButton(it.name.toLowerCase().capitalize()) }
  private val verticalAlignmentButtons =
    PopupVerticalAlignment.values().associate { it to JRadioButton(it.name.toLowerCase().capitalize()) }
  ...
  override fun isModified() =
    isDigitsOnly(fontSizeField.text) && (fontSizeField.text != configuration.configuration.fontSize.toString())
  ...
  override fun apply() {
    ...
    configuration.configuration.mainKeymap = mainKeymapPanel.getDescription()
    configuration.configuration.alternativeKeymap = getAlternativeKeymap()
    configuration.configuration.horizontalAlignment =
      horizontalAlignmentButtons.entries.find { it.value.isSelected }!!.key
  ...
  override fun reset() {
    ...
    horizontalAlignmentButtons.forEach { value, button ->
      button.isSelected = configuration.configuration.horizontalAlignment == value
    }
    verticalAlignmentButtons.forEach { value, button ->
      button.isSelected = configuration.configuration.verticalAlignment == value
    }
